...
public sealed partial class ChannelsMenu : DefaultWindow
{
    ...
    public void Populate(InstrumentComponent? instrument)
    {
        ...
        {
            if (instrument is null || instrument.Renderer is null)
            {
                continue;
            }
            var instProgram = instrument.Renderer.MidiProgram.ToString();
            var locString = "instruments-component-channel-instrument-gm-" + instProgram;
            var item = ChannelList.AddItem(_owner.Loc.GetString(locString), null, true, i);
